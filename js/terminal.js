(function () {
  ...
  function cmdCerts() {
    ...
    data.certs.forEach((c) => {
      const isActive = c.status === "Studying";
      const dot = isActive
        ? `<span class="roadmap-dot active">&#9654;</span>`
        : `<span class="roadmap-dot planned">&#9675;</span>`;
      const tag = isActive
        ? `<span class="roadmap-status-tag studying">${c.status}</span>`
        : `<span class="roadmap-status-tag planned">${c.status}</span>`;
